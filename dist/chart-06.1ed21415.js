...
var container = d3.select('#chart-06'); // Create our scales
...
var formater = d3.format('.2f'); // Create a line generator
...
var line2 = d3.area().x(function (d) {
  return xPositionScale(d.Age);
}).y0(height).y1(function (d) {
...
function ready(datapoints) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('path').datum(datapoints).attr('d', function (d) {
    ...
    svg.append('text').attr('x', width / 2).attr('y', 0 - margin.top / 2).attr('text-anchor', 'middle').style('font-size', '10px').text(name);
    svg.append('text').attr('x', 30).attr('y', 20).attr('text-anchor', 'middle').style('font-size', '10px').attr('fill', 'aqua').text(formater(d3.sum(d.values, function (d) {
      return +d.ASFR_jp;
    })));
    svg.append('text').attr('x', 40).attr('y', 30).attr('text-anchor', 'middle').style('font-size', '10px').attr('fill', 'red').text(formater(d3.sum(d.values, function (d) {
      return +d.ASFR_us;
    })));

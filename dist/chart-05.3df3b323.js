...
var width = 600 - margin.left - margin.right; // Add your svg
...
var colorScale = d3.scaleOrdinal().range(['#fbb4ae', '#b3cde3', '#ccebc5', '#decbe4', '#fed9a6', '#ffffcc', '#e5d8bd', '#fddaec', '#f2f2f2']);
var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().domain([180, 340]).range([height, 0]); // Create a d3.line function that uses your scales
...
function ready(datapoints) {
  ...
  xPositionScale.domain([dateMin, dateMax]); // Group your data together

  var nested = d3.nest().key(function (d) {
    return d.region;
  }).entries(datapoints);
  console.log(nested); // Draw your lines

  svg.selectAll('path').data(nested).enter().append('path') // .attr('stroke', 'black')
  .attr('fill', 'none').attr('d', function (d) {
    return line(d.values);
  }).attr('stroke', function (d) {
    return colorScale(d.key);
  }).attr('stroke-width', 3); // Add your text on the right-hand side

  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('x', width).attr('y', function (d) {
    var datapoints = d.values;
    var July = datapoints.find(function (d) {
      return d.month === 'July-17';
    });
    return yPositionScale(July.price);
  }).attr('font-size', 12).attr('alignment-baseline', 'middle').attr('dx', 8).attr('fill', 'black').attr('dy', function (d) {
    // Only push these two cities down some
    if (d.key === 'U.S.' || d.key === 'West South Central' || d.key === 'South Atlantic') {
      return -5;
    }
  ...
  nested.forEach(function (d) {
    ...
    var last = d.values[d.values.length - d.values.length];
  ...
  svg.data(nested).append('rect').lower().attr('fill', 'blue').attr('opacity', 0.1).attr('width', function (d) {
    var datapoints = d.values;
    var Dec = datapoints.find(function (d) {
      return d.month === 'December-16';
    });
    var Feb = datapoints.find(function (d) {
      return d.month === 'February-17';
    });
    console.log('const Dec is', Dec);
    console.log('const Feb is', Feb); // xScale(d.end_dt) - xScale(d.start_dt)

    return xPositionScale(Feb.datetime) - xPositionScale(Dec.datetime);
  }).attr('height', height) // const datapoints = d.values

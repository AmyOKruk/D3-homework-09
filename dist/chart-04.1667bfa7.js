...
var svg = d3.select('#chart-04').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create our scales

var xPositionScale = d3.scaleLinear().domain([2000, 2014]).range([0, width]);
...
function ready(datapoints) {
  ...
  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    ...
    var datapoints = d.values; // var decData = datapoints[datapoints.length - 1]
    // find datapoint for month 12

    var decData = datapoints.find(function (d) {
      return +d.Year === 2014;
    });
    return yPositionScale(decData.Value);
  }).attr('font-size', 12).attr('alignment-baseline', 'middle').attr('dx', 8).attr('fill', function (d) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('fill', 'none') // .attr('stroke', 'black')
